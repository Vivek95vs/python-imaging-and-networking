...
#         self.root = root
#         self.root.title("DICOM Server Push Utility with Resume Capability")
...
#                 'selected_folders': [f['rel_path'] for f in self.get_selected_folders() if f['selected']]
#             }
#
...
#                 self.log(f"Failed: {state['stats']['failed']} files", 'info')
#
...
#
#         # Title
#         title_label = ttk.Label(main_frame, text="DICOM Server Push Utility with Resume",
#                                 font=('Arial', 16, 'bold'))
...
#         server_frame.grid(row=0, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
...
#         local_frame.grid(row=1, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
...
#
#         ttk.Label(list_frame, text="Found DICOM Folders:").pack(anchor=tk.W)
#
#         # Treeview for folders
#         columns = ('select', 'folder', 'files', 'modalities', 'status')
...
#         self.folder_tree.column('folder', width=300)
...
#         sel_frame = ttk.Frame(source_frame)
#         sel_frame.grid(row=3, column=1, sticky=tk.W, pady=5)
#
#         ttk.Button(sel_frame, text="Select All", command=self.select_all).pack(side=tk.LEFT, padx=2)
#         ttk.Button(sel_frame, text="Deselect All", command=self.deselect_all).pack(side=tk.LEFT, padx=2)
#
...
#         button_frame1.grid(row=5, column=0, columnspan=3, pady=5)
#
#         self.push_btn = ttk.Button(button_frame1, text="▶ PUSH SELECTED",
#                                    command=self.start_push, width=20, style='Accent.TButton')
...
#         button_frame2.grid(row=6, column=0, columnspan=3, pady=5)
#
...
#
#         self.clear_btn = ttk.Button(button_frame2, text="🗑 Clear Selection",
...
#         self.save_cfg_btn.grid(row=0, column=2, padx=5)
#
#         # Store folder data
#         self.folder_items = []
#         self.folder_data = []
#         self.folder_modalities = {}
#
...
#         return dicom_folders, modalities_found
#
...
#
#             folders, modalities = self.find_dicom_folders(source)
#
#             if folders:
#                 self.root.after(0, lambda: self.update_folder_list(folders))
#                 total_files = sum(f['file_count'] for f in folders)
#                 self.log(f"\n✅ Found {len(folders)} DICOM folders with {total_files} files", 'success')
...
#         """Update the treeview with found folders"""
#         for folder in folders:
#             item_id = self.folder_tree.insert('', tk.END,
#                                               values=('☐', folder['rel_path'], folder['file_count'],
#                                                       folder['modalities'], 'Pending'))
#             self.folder_items.append(item_id)
...
#                 'modalities': folder['modalities'],
#                 'status': 'Pending'
#             })
#
#         self.stats['total_folders'] = len(folders)
#         self.stats['total_files'] = sum(f['file_count'] for f in folders)
#
...
#         """Test connection to server with C-ECHO"""
#         self.log("\n" + "=" * 60, 'header')
...
#
#                 self.root.after(0, lambda: self.log("Establishing association...", 'info'))
#
...
#                     if status:
#                         self.root.after(0,
#                                         lambda: self.log("✅ Connection successful - Server is responding", 'success'))
#                         self.root.after(0, lambda: messagebox.showinfo("Success", "Connection to server successful!"))
#                     else:
#                         self.root.after(0, lambda: self.log("❌ C-ECHO failed", 'error'))
#                 else:
#                     self.root.after(0, lambda: self.log("❌ Could not establish association with server", 'error'))
#
#             except Exception as e:
#                 self.root.after(0, lambda: self.log(f"❌ Connection error: {str(e)}", 'error'))
#             finally:
...
#                                    f"Push {len(selected)} folders with {total_files} files to server?\n\n"
...
#         selected = [d for d in self.folder_data if d['rel_path'] in state.get('selected_folders', [])]
#
#         if not selected:
...
#             all_verifications = []
#
#             # Determine starting folder index for resume
#             start_idx = 0
#             if is_resume:
#                 start_idx = self.push_state['current_folder_index']
#
#             for idx, folder in enumerate(selected_folders[start_idx:], start_idx + 1):
#                 if self.stop_push:
#                     break
#
#                 folder_path = folder['path']
#                 self.push_state['current_folder_index'] = idx - 1
#                 self.push_state['current_folder_path'] = str(folder_path)
#
#                 self.root.after(0, lambda i=idx, t=len(selected_folders), p=folder['rel_path']:
#                 self.stats_label.config(text=f"Folder {i}/{t}: {p}"))
#
#                 self.log(f"\n📁 [{idx}/{len(selected_folders)}] Processing: {folder['rel_path']}", 'header')
...
#                     self.folder_tree.set(folder['id'], 'status', 'Completed')
#
#                 # Update stats
