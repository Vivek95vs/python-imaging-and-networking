...
class Contour:
    def __init__(self, name: str, color: Tuple[int, int, int] = (255, 0, 0)):
        ...
        self.contours = {}  # Dictionary: slice_idx -> list of contour polygons
        self.interpolated_contours = {}  # Store interpolated contours
    ...
    def add_contour(self, slice_idx: int, polygon: np.ndarray):
        ...
        self.interpolated_contours = {}  # Clear interpolation cache
    ...
    def interpolate_between_slices(self, all_slice_indices, ct_slices):
        """Interpolate contours between drawn slices to create smooth 3D volume"""
        if not self.contours:
            return {}

        # Get slices that have contours
        drawn_slices = sorted(self.contours.keys())

        if len(drawn_slices) <= 1:
            # If only one slice, just duplicate it to adjacent slices
            single_slice = drawn_slices[0]
            contours_at_slice = self.contours[single_slice]

            # Find range to interpolate
            min_slice = max(0, single_slice - 5)
            max_slice = min(len(all_slice_indices) - 1, single_slice + 5)

            for slice_idx in range(min_slice, max_slice + 1):
                if slice_idx != single_slice:
                    if slice_idx not in self.interpolated_contours:
                        self.interpolated_contours[slice_idx] = []
                    for contour in contours_at_slice:
                        # Scale contour based on distance from original slice
                        distance = abs(slice_idx - single_slice)
                        scale_factor = 1.0 - (distance * 0.1)  # Reduce size gradually
                        if scale_factor > 0.3:
                            scaled_contour = self.scale_contour(contour, scale_factor)
                            self.interpolated_contours[slice_idx].append(scaled_contour)

            self.interpolated_contours.update(self.contours)
            return self.interpolated_contours

        # Multiple slices - perform proper interpolation
        # Create a binary mask for each drawn slice
        mask_3d = {}
        for slice_idx in drawn_slices:
            mask = np.zeros((ct_slices[slice_idx].pixel_array.shape[0],
                             ct_slices[slice_idx].pixel_array.shape[1]), dtype=bool)
            for polygon in self.contours[slice_idx]:
                rr, cc = self.polygon_to_mask(polygon, mask.shape)
                mask[rr, cc] = True
            mask_3d[slice_idx] = mask

        # Interpolate between slices
        all_slice_indices_list = list(range(len(all_slice_indices)))

        for target_slice in all_slice_indices_list:
            if target_slice in self.contours:
                continue  # Skip slices that already have contours

            # Find nearest drawn slices above and below
            above = [s for s in drawn_slices if s < target_slice]
            below = [s for s in drawn_slices if s > target_slice]

            if above and below:
                # Interpolate between two slices
                above_slice = max(above)
                below_slice = min(below)

                # Calculate interpolation weight
                total_distance = below_slice - above_slice
                weight_above = (below_slice - target_slice) / total_distance
                weight_below = (target_slice - above_slice) / total_distance

                # Get contours from both slices
                contours_above = self.contours[above_slice]
                contours_below = self.contours[below_slice]

                # Interpolate contours
                if len(contours_above) == len(contours_below):
                    for i in range(len(contours_above)):
                        interpolated = self.interpolate_contours(
                            contours_above[i], contours_below[i],
                            weight_above, weight_below
                        )
                        if target_slice not in self.interpolated_contours:
                            self.interpolated_contours[target_slice] = []
                        self.interpolated_contours[target_slice].append(interpolated)

            elif above:
                # Extrapolate from above slice
                above_slice = max(above)
                distance = target_slice - above_slice
                scale_factor = 1.0 - (distance * 0.05)
                if scale_factor > 0.3:
                    for contour in self.contours[above_slice]:
                        scaled = self.scale_contour(contour, scale_factor)
                        if target_slice not in self.interpolated_contours:
                            self.interpolated_contours[target_slice] = []
                        self.interpolated_contours[target_slice].append(scaled)

            elif below:
                # Extrapolate from below slice
                below_slice = min(below)
                distance = below_slice - target_slice
                scale_factor = 1.0 - (distance * 0.05)
                if scale_factor > 0.3:
                    for contour in self.contours[below_slice]:
                        scaled = self.scale_contour(contour, scale_factor)
                        if target_slice not in self.interpolated_contours:
                            self.interpolated_contours[target_slice] = []
                        self.interpolated_contours[target_slice].append(scaled)
    ...
    def polygon_to_mask(self, polygon, shape):
        ...
        rr, cc = draw_polygon(polygon[:, 1], polygon[:, 0], shape)
        return rr, cc

    def interpolate_contours(self, contour1, contour2, weight1, weight2):
        """Interpolate between two contours"""
        # Ensure both contours have the same number of points
        n_points = max(len(contour1), len(contour2))

        # Interpolate points
        interpolated = []
        for i in range(n_points):
            # Get corresponding points
            idx1 = int(i * len(contour1) / n_points) if len(contour1) > 0 else 0
            idx2 = int(i * len(contour2) / n_points) if len(contour2) > 0 else 0

            point1 = contour1[idx1] if idx1 < len(contour1) else contour1[-1]
            point2 = contour2[idx2] if idx2 < len(contour2) else contour2[-1]

            # Weighted average
            x = point1[0] * weight1 + point2[0] * weight2
            y = point1[1] * weight1 + point2[1] * weight2
            interpolated.append([x, y])

        return np.array(interpolated)

    def scale_contour(self, contour, scale_factor):
        """Scale contour relative to its center"""
        center = np.mean(contour, axis=0)
        scaled = center + (contour - center) * scale_factor
        return scaled
...
class CTViewer(QMainWindow):
    def __init__(self):
        ...
        self.zoom_level = 1.0
        self.log_messages = []
        self.interpolation_enabled = True  # New flag for interpolation

        self.init_ui()

    def init_ui(self):
        self.setWindowTitle('CT Contour Drawing Tool - With 3D Interpolation')
        self.setGeometry(100, 100, 1500, 900)

        # Set style (same as before)
        self.setStyleSheet("""
        ...
        self.log_message("Application started with 3D interpolation support")
    ...
    def create_left_panel(self):
        ...
        self.interpolation_cb = QCheckBox("Enable 3D Interpolation between slices")
        self.interpolation_cb.setChecked(True)
        self.interpolation_cb.stateChanged.connect(self.toggle_interpolation)
        file_layout.addWidget(self.interpolation_cb)

        self.save_rt_btn = QPushButton("2. Save as RT Structure (with interpolation)")
        ...
        # Structure management (same as before)
        ...
        # Display controls (same as before)
        ...
        info_text = QLabel(
            ...
            "6. Draw contours on KEY slices (top, middle, bottom)\n"
            "7. Enable interpolation for automatic filling\n"
            "8. Navigate slices with slider or arrow keys\n"
            "9. Click 'Save as RT Structure'"
        )
    ...
    def create_center_panel(self):
        ...
        self.figure = plt.figure(figsize=(12, 10), dpi=100, facecolor='#2b2b2b')
    ...
    def toggle_interpolation(self):
        self.interpolation_enabled = self.interpolation_cb.isChecked()
        if self.interpolation_enabled:
            self.log_message("3D Interpolation enabled - Contours will be automatically filled between slices")
        else:
            self.log_message("3D Interpolation disabled - Only drawn slices will be saved")
        self.update_display()
    ...
    def update_summary(self):
        ...
        if not self.contours:
            ...
            for name, contour in self.contours.items():
                ...
                summary += f"   Drawn slices: {slices_with_contours}\n"
                summary += f"   Drawn contours: {total_contours}\n"

                if self.interpolation_enabled and slices_with_contours > 0:
                    # Calculate interpolated slices
                    min_slice = min(contour.contours.keys())
                    max_slice = max(contour.contours.keys())
                    interpolated_count = (max_slice - min_slice + 1) - slices_with_contours
                    summary += f"   Interpolated slices: {interpolated_count}\n"
                    summary += f"   Total slices in volume: {max_slice - min_slice + 1}\n"
    ...
    def change_color(self):
        ...
        if color.isValid():
            self.current_contour.color = [color.red(), color.green(), color.blue()]
            self.color_preview.setStyleSheet(
                f"background-color: rgb({self.current_contour.color[0]}, {self.current_contour.color[1]}, {self.current_contour.color[2]}); border: 1px solid white;")
    ...
    def update_display(self):
        ...
        # Draw contours (use interpolated if enabled)
        for contour_name, contour_obj in self.contours.items():
            # Get contours for current slice (original or interpolated)
            contours_to_draw = []

            if self.interpolation_enabled and len(contour_obj.contours) > 0:
                # Use interpolated contours if available
                if not contour_obj.interpolated_contours:
                    contour_obj.interpolate_between_slices(self.ct_slices, self.ct_slices)

                if self.current_slice in contour_obj.interpolated_contours:
                    contours_to_draw = contour_obj.interpolated_contours[self.current_slice]
            else:
                # Use only original contours
                if self.current_slice in contour_obj.contours:
                    contours_to_draw = contour_obj.contours[self.current_slice]

            # Draw contours
            for polygon in contours_to_draw:
    ...
    def save_rt_structure(self):
        """Save as DICOM RT Structure with interpolation"""
        if not self.ct_slices:
        ...
        try:
            # Generate interpolated contours if enabled
            if self.interpolation_enabled:
                self.log_message("Generating interpolated contours for 3D volume...")
                for contour_obj in self.contours.values():
                    contour_obj.interpolate_between_slices(self.ct_slices, self.ct_slices)
                self.log_message("Interpolation completed")

            # Create RT Structure with interpolated contours
            self.create_rt_structure_with_interpolation(file_path)
            ...
            QMessageBox.information(self, "Success",
                                    ...
                                    f"Interpolation: {'Enabled' if self.interpolation_enabled else 'Disabled'}\n\n"
                                    f"The file will show smooth 3D contours in sagittal and coronal views.")
    ...
    def create_rt_structure_with_interpolation(self, filename):
        """Create RT Structure using interpolated contours"""

        self.log_message("Creating DICOM RT Structure Set with interpolation...")
        ...
        # Patient information
        ds.PatientName = first_ct.get('PatientName', 'Unknown^Patient')
        ...
        ds.SeriesNumber = 200
        ds.SeriesDescription = 'RT Structure Set with Interpolation'
        ...
        # Manufacturer
        ds.Manufacturer = 'Python CT Contour Tool with 3D Interpolation'
        ds.ManufacturerModelName = 'RT Structure Creator v2.0'
        ...
        for contour_name, contour_obj in self.contours.items():
            ...
            # Determine which contours to use (original or interpolated)
            contours_to_use = contour_obj.interpolated_contours if self.interpolation_enabled and contour_obj.interpolated_contours else contour_obj.contours

            if not contours_to_use:
                self.log_message(f"  Warning: No contours found for {contour_name}, skipping")
                continue
            ...
            structure_roi.ROIGenerationAlgorithm = 'MANUAL' if not self.interpolation_enabled else 'INTERPOLATED'
            ds.StructureSetROISequence.append(structure_roi)

            # ROI Contour
            roi_contour = Dataset()
            roi_contour.ReferencedROINumber = roi_number
            roi_contour.ROIDisplayColor = contour_obj.color
            roi_contour.ContourSequence = []

            # Add contours for each slice
            for slice_idx, polygons in contours_to_use.items():
            ...
            if roi_contour.ContourSequence:
                ...
                self.log_message(
                    f"  ✓ Added {len(roi_contour.ContourSequence)} contours for {contour_name} across {len(contours_to_use)} slices")
            else:
        ...
        try:
            ...
            for contour_name, contour_obj in self.contours.items():
                ...
                for slice_idx, polygons in contour_obj.contours.items():
                    structure_data['drawn_contours'][str(slice_idx)] = [polygon.tolist() for polygon in polygons]

                # Save interpolated contours if available
                if contour_obj.interpolated_contours:
                    for slice_idx, polygons in contour_obj.interpolated_contours.items():
                        structure_data['interpolated_contours'][str(slice_idx)] = [polygon.tolist() for polygon in
                                                                                   polygons]
